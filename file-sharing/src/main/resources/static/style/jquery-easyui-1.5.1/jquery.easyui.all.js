        ...
        loader: function (param, success, error) {
            ...
            Utils.doAjax({
                ...
                success: function (data) {
                    ...
                    data = $.trim(data);
                    if (data.indexOf("<!DOCTYPE html>") === 0) { // 登录超时判断
...
(function ($) {
    ...
    function createDialog(options) {
        ...
        /*Start修改代码:解决IE浏览器下按Enter键查询触发确定按钮关闭弹窗*/
        var buttonObj = win.find('.dialog-button').addClass('messager-button').find('a:first');
        setTimeout(function () {
            buttonObj.focus();
        }, 0);
        /*End*/
...
(function ($) {
    ...
    function init(container) {
        ...
        function _add(cc) {
            ...
            cc.children('div').each(function () {
                var popts = $.fn.layout.parsePanelOptions(this);
                if ('north,south,east,west,center'.indexOf(popts.region) >= 0) {
                    addPanel(container, popts, this);
                }
            });
            opts.onAdd = onAdd;
        }
...
//			bindEvents(this);
    ...
    $.fn.validatebox.defaults = {
        ...
        }	// fires when validation completes
    };
})(jQuery);
/**
        ...
        if (opts.iconClear) {	//author zsk iconClear
            bb.unshift({
                ...
                handler: function (e) {
                    ...
                    var $target = $(target);
                    var opts = $target.data();
                    var fieldTypes = $.fn.form.defaults.fieldTypes;
                    // 根据继承顺序判断调用相应的clear方法
                    for (var i = 0, j = fieldTypes.length; i < j; i++) {
                        if (opts[fieldTypes[i]]) {
                            $target[fieldTypes[i]]('clear');
                            break;
                        }
                    }
...
(function ($) {
    ...
    function setValues(target, values, remainText) {
        ...
        };	// prevent from triggering onChange event
...
(function ($) {
    ...
    $.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults, {
        ...
        buttons: [{
            ...
            handler: function (target) {
                $(target).combo('setText', '').combo('setValue', '');
                $(target).datetimebox('hidePanel');
            }
        }, {
            text: function (target) {
